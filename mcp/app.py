...
def exercises():
    ...
    # Add a prefix to ensure the exercise search tool is used
    exercise_query = f"Show me {query_text} exercises"
    
    # Get response from fitness assistant
    response = query_fitness_assistant(exercise_query)
    
    # Save the query and response (using a default user ID for now)
    save_user_query('default_user', exercise_query, response)
